...
class LyraDiffVaeModel():
    ...
    def __init__(
        ...
        self.is_dynamic_tiling_s3diff = 1==11
    ...
    def encode(
        ...
        if not self.is_dynamic_tiling_s3diff:
            ...
            if self.use_tiling and max(H, W) > self.pad * 2 + self.tile_size:
                ...
                return self.tiled_encode_s3diff(x, return_dict=return_dict)
            else:
    ...
    def _decode(self, z: torch.FloatTensor, return_dict: bool = True) -> torch.FloatTensor:
        if not self.is_dynamic_tiling_s3diff:
            ...
            if self.use_tiling and max(H, W) > self.pad * 2 + self.tile_size:
                ...
                return self.tiled_decode_s3diff(z, return_dict=return_dict)
            else:
    ...
    def tiled_decode_s3diff(self, z: torch.FloatTensor, return_dict: bool = True) -> torch.FloatTensor:
        r"""
        Decode a batch of images using a tiled decoder.
        ...
        Returns:
            [`~models.vae.DecoderOutput`] or `tuple`:
                ...
                returned.
        """
        # np.save(f"//data2/home/kiokaxiao/data/sd3diff/z_ascend.npy", z.detach().cpu().numpy())
        ...
        for i in range(len(in_bboxes)):
            ...
            # print("decode tile size: ", tile.shape)
            if result is None:
                # result = torch.zeros((N, tile.shape[1], height * 8 if is_decoder else height // 8, width * 8 if is_decoder else width // 8), requires_grad=False).cuda()
            ...
            # print("decoded_crop", decoded_crop.shape)

            result[:, :, out_bboxes[i][2]:out_bboxes[i][3], out_bboxes[i][0]:out_bboxes[i][1]] = decoded_crop

            # np.save(f"/data2/home/kiokaxiao/data/sd3diff/tile_inp_{i}_ascend.npy", tile.detach().cpu().numpy())
            # np.save(f"/data2/home/kiokaxiao/data/sd3diff/tile_{i}_ascend.npy", decoded_crop.detach().cpu().numpy())


        return DecoderOutput(sample=result)


    def tiled_encode_s3diff(self, x: torch.FloatTensor, return_dict: bool = True) -> DiagonalGaussianDistribution:
        r"""Encode a batch of images using a tiled encoder.
        ...
        for i in range(len(in_bboxes)):
            ...
            # print("decoded", decoded.shape, crop_decoded.shape)
            tiles.append(tile)

            # print("{}th, decoded bound: [{} {} {} {}], result shape: {}, tile shape: {}, decoded: {}, crop_decoded.shape:{}".format(
                # i, out_bboxes[i][2], out_bboxes[i][3], out_bboxes[i][0], out_bboxes[i][1], 
                # result.shape, tile.shape, decoded.shape, crop_decoded.shape))
            result[:, :, out_bboxes[i][2]:out_bboxes[i][3], out_bboxes[i][0]:out_bboxes[i][1]] = crop_decoded
            # print("set result ok {}, total:{}".format(i, len(in_bboxes)))

            # import numpy as np
            # np.save(f"/data2/home/kiokaxiao/data/sd3diff/tile_inp_{i}_ascend.npy", tile.detach().cpu().numpy())
            # np.save(f"/data2/home/kiokaxiao/data/sd3diff/tile_{i}_ascend.npy", crop_decoded.detach().cpu().numpy())
