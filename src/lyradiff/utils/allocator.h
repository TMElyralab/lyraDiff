...
public:
    ...
    virtual size_t       getAllSize()                                                             = 0;
    virtual void         printAllNameSize()                                                       = 0;
    virtual void         freeAllNameBuf()                                                         = 0;
    virtual void         freeWithName(const std::string& buf_name, bool is_host = false)          = 0;
    virtual void*
    reMallocWithName(std::string buf_name, size_t size, const bool is_set_zero = true, bool is_host = false) = 0;
...
private:
    ...
    std::unordered_map<std::string, void*>* name_pointer_mapping_;
...
public:
    Allocator(int device_id, cudaStream_t stream = 0): device_id_(device_id), stream_(stream)
    {
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        pointer_mapping_      = new std::unordered_map<void*, size_t>();
        name_pointer_mapping_ = new std::unordered_map<std::string, void*>();
#if defined(CUDA_MEMORY_POOL_DISABLED)
        FT_LOG_WARNING(
    ...
    void* iReMalloc(void* ptr, size_t size, const bool is_set_zero = true, bool is_host = false)
    {
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        size              = ((size + 31) / 32) * 32;  // make the buffer align with 32 bytes
        void* void_ptr    = (void*)ptr;
    ...
    void* reMallocWithName(std::string buf_name, size_t size, const bool is_set_zero = true, bool is_host = false)
    {
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);

        // 没找到代表需要重新创建指针
        if (name_pointer_mapping_->find(buf_name) == name_pointer_mapping_->end()) {
            void* new_ptr                      = iReMalloc(nullptr, size, is_set_zero, is_host);
            (*name_pointer_mapping_)[buf_name] = new_ptr;
            return new_ptr;
        }

        void* cur_ptr = name_pointer_mapping_->at(buf_name);

        // 如果现有的buf 小于我们需要的大小，则remalloc 并更新指针
        if (pointer_mapping_->at(cur_ptr) < size) {
            cur_ptr                             = iReMalloc(cur_ptr, size, is_set_zero, is_host);
            name_pointer_mapping_->at(buf_name) = cur_ptr;
        }
        // 如果现有的buf 大于等于我们需要的大小，则啥都不需要做

        return cur_ptr;
    }

    void freeWithName(const std::string& buf_name, bool is_host = false)
    {
        if (name_pointer_mapping_->find(buf_name) == name_pointer_mapping_->end())
            return;
        auto ptr = name_pointer_mapping_->at(buf_name);
        if (is_host) {
        ...
        name_pointer_mapping_->erase(buf_name);
        pointer_mapping_->erase(ptr);
    }

    size_t getAllSize()
    {
        size_t all_size = 0;
        for (auto it = pointer_mapping_->begin(); it != pointer_mapping_->end(); it++) {
            all_size += it->second;
        }
        return all_size;
    }

    void printAllNameSize()
    {
        size_t all_size = 0;
        for (auto it = name_pointer_mapping_->begin(); it != name_pointer_mapping_->end(); it++) {
            std::string cur_name = it->first;
            size_t      cur_size = pointer_mapping_->at(it->second);
            all_size += cur_size;
            std::cout << "cur name " << cur_name << " with size " << cur_size / 1024 / 1024 << "MBs" << std::endl;
        }
        std::cout << "overall size " << all_size / 1024 / 1024 << "MBs" << std::endl;
    }
