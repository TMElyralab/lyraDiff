...
void cublasAlgoMap::loadGemmConfig()
{
    ...
    while (fscanf(fd,
        ...
        //     continue;
        // }
        cublasAlgoConfig_t markStr{batchCount2, n2, m2, k2, static_cast<CublasDataType>(dataType)};
        // printf("cur line m n k %d %d %d \n", m2, n2, k2);
...
{
    int next_n = nextPowerOfTwo(n);

    cublasAlgoConfig_t mark{batch_count, next_n, m, k, data_type};
...
void cublasAlgoMap::insertShape(const int batch_count, const int m, const int n, const int k)
{
    cublasAlgoConfig_t shape{batch_count, m, n, k, static_cast<CublasDataType>(HALF_DATATYPE)};
    shape_set_.insert(shape);
}

void cublasAlgoMap::printAllShape()
{
    for (auto i = shape_set_.begin(); i != shape_set_.end(); ++i) {
        printf("cublasAlgoMap current gemm m: %d, n: %d, k: %d \n", i->m, i->n, i->k);
    }
}
