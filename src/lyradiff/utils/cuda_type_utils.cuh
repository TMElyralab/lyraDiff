...
template<typename T>
struct TypeConverter4 {
    using Type = half4;
};  // keep for generality

template<>
struct TypeConverter4<half4> {
    using Type = half;
};

template<>
struct TypeConverter4<half> {
    using Type = half4;
};

template<>
struct TypeConverter4<float4> {
    using Type = float;
};

template<>
struct TypeConverter4<float> {
    using Type = float4;
};
...
template<typename T>
inline __device__ T mul(T a, T b)
{
    return a * b;
}

template<>
inline __device__ half mul(half a, half b)
{
    return __hmul(a, b);
}

template<>
inline __device__ half2 mul(half2 a, half2 b)
{
    return __hmul2(a, b);
}
...
template<>
__device__ inline double2 cuda_cast<double2, float2>(float2 val)
{
    return make_double2(val.x, val.y);
}

template<>
__device__ inline double2 cuda_cast<double2, half2>(half2 val)
{
    return make_double2(__half2float(val.x), __half2float(val.y));
}

template<>
__device__ inline double2 cuda_cast<double2, half>(half val)
{
    return make_double2(__half2float(val), __half2float(val));
}

template<>
__device__ inline double2 cuda_cast<double2, float>(float val)
{
    return make_double2(val, val);
}

template<>
__device__ inline double cuda_cast<double, half>(half val)
{
    return __half2float(val);
}

template<>
__device__ inline half cuda_cast<half, double>(double val)
{
...
template<>
__device__ inline float2 cuda_cast<float2, double2>(double2 val)
{
    return make_float2(val.x, val.y);
}

template<>
__device__ inline half2 cuda_cast<half2, double2>(double2 val)
{
    return make_half2(__double2half(val.x), __double2half(val.y));
}
...
inline __device__ float clamp_half(float f)
{
    return f;
}
inline __device__ half clamp_half(half f)
{
    return __hmin(65504.0, __hmax(f, -65504.0));
}
#ifdef ENABLE_BF16
inline __device__ __nv_bfloat16 clamp_half(__nv_bfloat16 f)
{
    return f;
}
#endif
