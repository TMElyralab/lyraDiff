...
{
    LYRA_CHECK_WITH_INFO(size >= ((size_t)0), "Ask deviceMalloc size " + std::to_string(size) + "< 0 is invalid.");
...
template void invokeCudaCast(half* dst, float const* const src, const size_t size, cudaStream_t stream);
...
template void invokeCudaD2DcpyConvert(__nv_bfloat16* tgt, const half* src, const size_t size, cudaStream_t stream);
...
template void invokeCudaD2DcpyConvert(half* tgt, const __nv_bfloat16* src, const size_t size, cudaStream_t stream);
template void invokeCudaD2DcpyConvert(int* tgt, const __nv_bfloat16* src, const size_t size, cudaStream_t stream);
#endif  // ENABLE_BF16
#ifdef ENABLE_FP8
template void invokeCudaD2DcpyConvert(__nv_fp8_e4m3* tgt, const float* src, const size_t size, cudaStream_t stream);
template void invokeCudaD2DcpyConvert(__nv_fp8_e4m3* tgt, const half* src, const size_t size, cudaStream_t stream);
template void
invokeCudaD2DcpyConvert(__nv_fp8_e4m3* tgt, const __nv_bfloat16* src, const size_t size, cudaStream_t stream);
template void invokeCudaD2DcpyConvert(float* tgt, const __nv_fp8_e4m3* src, const size_t size, cudaStream_t stream);
#endif  // ENABLE_FP8
...
template void invokeCudaD2DScaleCpyConvert(__nv_fp8_e4m3* tgt, const float* src, const float* scale, bool invert_scale, const size_t size, cudaStream_t stream);
template void invokeCudaD2DScaleCpyConvert(__nv_fp8_e4m3* tgt, const half* src, const float* scale, bool invert_scale, const size_t size, cudaStream_t stream);
template void invokeCudaD2DScaleCpyConvert(__nv_fp8_e4m3* tgt, const __nv_bfloat16* src, const float* scale, bool invert_scale, const size_t size, cudaStream_t stream);
#endif  // ENABLE_FP8
// clang-format on
...
void saveToBinary(const T* ptr, const size_t size, std::string filename)
{
    ...
    LYRA_CHECK_WITH_INFO(out.is_open(), "Fail to open file " + filename);
...
template void invokeInPlaceTranspose(half* data, half* workspace, const int dim0, const int dim1);
#ifdef ENABLE_BF16
...
template<typename T>
__global__ void calculateSum(const T* data, float* sum, const size_t size)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < size) {
        float localSum = 0.0f;
        for (int i = idx; i < size; i += blockDim.x * gridDim.x) {
            localSum += cuda_cast<float>(data[i]);
        }
        atomicAdd(sum, localSum);  // 使用原子操作避免竞态条件
    }
}
...
void invokeCalculateSum(const T* data, float* sum, const size_t size)
{
    ...
    // 启动内核
    calculateSum<<<gridSize, blockSize>>>(data, sum, size);
}

template void invokeCalculateSum(const half* data, float* sum, const size_t size);
template void invokeCalculateSum(const float* data, float* sum, const size_t size);
