#include "GLVUpBlock2d.h"
#include "src/lyradiff/kernels/cross_attn_upblock_2d/cat_kernels.h"

using namespace std;
namespace lyradiff {
template<typename T>
GLVUpBlock2d<T>::GLVUpBlock2d(const size_t     in_channels,
...
template<typename T>
GLVUpBlock2d<T>::GLVUpBlock2d(GLVUpBlock2d<T> const& up_block2d):
    ...
    stream_assistant_(up_block2d.stream_assistant_)
{
    ...
    project_module1 = up_block2d.project_module1;
    project_module2 = up_block2d.project_module2;
    // project_module3 = up_block2d.project_module3;
}

template<typename T>
GLVUpBlock2d<T>::~GLVUpBlock2d()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void GLVUpBlock2d<T>::allocateBuffer()
{
    LYRA_CHECK_WITH_INFO(
        false, "GLVUpBlock2d::allocateBuffer() is deprecated. Use `allocateBuffer(size_t token_num, ...)` instead");
}
    ...
    size_t batch_size, size_t height, size_t width, size_t hidden_channel, size_t target_height, size_t target_width)
{
    ...
    cat_buf_0_ = (T*)allocator_->reMallocWithName("GLVUpBlock2d_cat_buf_0_", cat_size0, false);
    cat_buf_   = (T*)allocator_->reMallocWithName("GLVUpBlock2d_cat_buf_", cat_size, false);

    is_allocate_buffer_ = false;
}

template<typename T>
void GLVUpBlock2d<T>::freeBuffer()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void GLVUpBlock2d<T>::forward(std::vector<lyradiff::Tensor>*       output_tensors,
...
template<typename T>
void GLVUpBlock2d<T>::forward(TensorMap*                   output_tensors,
                              ...
                              const float                  control_scale)
{
    ...
    // project_module for supir controlnet
    ...
    if (input_tensors->isExist("round2_control") && weights->is_maintain_project_buffer) {
        ...
        TensorMap cur_input_map = TensorMap(
            {{"hidden_states", output}, {"cur_round_input", round2_input}, {"control_hidden_states", round2_control}});
    ...
    if (input_tensors->isExist("round3_control") && weights->is_maintain_project_buffer) {
        ...
        TensorMap cur_input_map = TensorMap(
            {{"hidden_states", output}, {"cur_round_input", round3_input}, {"control_hidden_states", round3_control}});

        TensorMap cur_output_map = TensorMap({{"output", cur_output}});

        project_module2->forward(&cur_output_map, &cur_input_map, weights->project_module_weight3, control_scale);
...
template class GLVUpBlock2d<float>;
template class GLVUpBlock2d<half>;
}  // namespace lyradiff
