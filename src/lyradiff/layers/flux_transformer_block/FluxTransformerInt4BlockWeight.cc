...
template<typename T>
FluxTransformerInt4BlockWeight<T>::FluxTransformerInt4BlockWeight(size_t        embedding_dim,
                                                                  ...
                                                                  IAllocator*   allocator)
{
    ...
    attn_weight = new FluxAttentionInt4ProcessorWeight<T>(
        embedding_dim_, embedding_head_num_, embedding_head_dim_, quant_level, allocator);
    post_attn_weight         = new FluxAttnPostInt4ProcessorWeight<T>(embedding_dim_, quant_level, allocator);
    context_post_attn_weight = new FluxAttnPostInt4ProcessorWeight<T>(embedding_dim_, quant_level, allocator);
...
template<typename T>
FluxTransformerInt4BlockWeight<T>::~FluxTransformerInt4BlockWeight()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void FluxTransformerInt4BlockWeight<T>::mallocWeights()
{
    if (!is_maintain_buffer) {
...
template<typename T>
void FluxTransformerInt4BlockWeight<T>::loadWeights(const std::string prefix, FtCudaDataType model_file_type)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void FluxTransformerInt4BlockWeight<T>::loadWeightsFromCache(std::string                             prefix,
...
template class FluxTransformerInt4BlockWeight<float>;
template class FluxTransformerInt4BlockWeight<half>;

#ifdef ENABLE_BF16
template class FluxTransformerInt4BlockWeight<__nv_bfloat16>;
#endif
}  // namespace lyradiff