#include "FluxTransformerFP8Block.h"
...
template<typename T>
FluxTransformerFP8Block<T>::FluxTransformerFP8Block(size_t              embedding_dim,
                            ...
                            quant_level)
{
    ...
    attn_processor = new FluxAttentionFP8Processor<T>(this->embedding_dim_,
                                                      ...
                                                      is_free_buffer_after_forward,
                                                      sparse);
    post_processor = new FluxAttnPostFP8Processor<T>(
        this->embedding_dim_, this->stream_, cublas_wrapper, allocator, is_free_buffer_after_forward, sparse);
}

template<typename T>
FluxTransformerFP8Block<T>::FluxTransformerFP8Block(FluxTransformerFP8Block<T> const& other):
...
template<typename T>
void FluxTransformerFP8Block<T>::allocateBuffer()
{
    LYRA_CHECK_WITH_INFO(
        false,
        "FluxTransformerFP8Block::allocateBuffer() is deprecated. Use `allocateBuffer(size_t batch_size, ...)` instead");
}

template<typename T>
void FluxTransformerFP8Block<T>::allocateBuffer(size_t batch_size, size_t seq_len, size_t context_seq_len)
{
...
template<typename T>
void FluxTransformerFP8Block<T>::freeBuffer()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
}

template<typename T>
void FluxTransformerFP8Block<T>::forward(const TensorMap*                        output_tensors,
...
template<typename T>
FluxTransformerFP8Block<T>::~FluxTransformerFP8Block()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    freeBuffer();
...
template class FluxTransformerFP8Block<float>;
template class FluxTransformerFP8Block<half>;
#ifdef ENABLE_BF16
template class FluxTransformerFP8Block<__nv_bfloat16>;
#endif
}  // namespace lyradiff