...
namespace lyradiff {
template<typename T>
GLVCrossAttnUpBlock2dWeight<T>::GLVCrossAttnUpBlock2dWeight(const size_t  in_channels,
    ...
    inner_trans_num_(inner_trans_num)
{
    ...
    cross_project_module_weight = new ZeroCrossAttnWeight<T>(out_channels_, in_channels_);
...
template<typename T>
GLVCrossAttnUpBlock2dWeight<T>::~GLVCrossAttnUpBlock2dWeight()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (is_maintain_buffer) {
        ...
        delete cross_project_module_weight;
        ...
        project_module_weight1        = nullptr;
        project_module_weight2        = nullptr;
        project_module_weight3        = nullptr;
        cross_project_module_weight   = nullptr;
    }
}

template<typename T>
void GLVCrossAttnUpBlock2dWeight<T>::mallocWeights()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (!is_maintain_buffer) {
...
template<typename T>
void GLVCrossAttnUpBlock2dWeight<T>::mallocProjectModuleWeights()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (!is_maintain_project_buffer) {
        ...
        cross_project_module_weight->mallocWeights();
...
template<typename T>
void GLVCrossAttnUpBlock2dWeight<T>::loadWeights(std::string prefix, FtCudaDataType model_file_type)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    // norm weights
    if (!is_maintain_buffer) {
...
template<typename T>
void GLVCrossAttnUpBlock2dWeight<T>::loadProjectWeights(std::string prefix, FtCudaDataType model_file_type)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    // norm weights
    if (!is_maintain_project_buffer) {
    ...
    cross_project_module_weight->loadWeights(prefix + std::string("project_modules.3."), model_file_type);
}

template<typename T>
void GLVCrossAttnUpBlock2dWeight<T>::loadProjectWeightsFromCache(std::string                             prefix,
                                                                 ...
                                                                 cudaMemcpyKind                          memcpyKind)
{
    ...
    cross_project_module_weight->loadWeightsFromCache(prefix + std::string("project_modules.3."), weights, memcpyKind);
}

template<typename T>
void GLVCrossAttnUpBlock2dWeight<T>::loadWeightsFromCache(std::string                             prefix,
                                                          std::unordered_map<std::string, void*>& weights,
                                                          cudaMemcpyKind                          memcpyKind)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void GLVCrossAttnUpBlock2dWeight<T>::mallocLoraBuffer()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (!is_maintain_lora) {
...
template<typename T>
void GLVCrossAttnUpBlock2dWeight<T>::loadLoraFromWeight(std::string                          lora_path,
                                                        std::string                          prefix,
...
template<typename T>
void GLVCrossAttnUpBlock2dWeight<T>::loadLoraFromCache(std::string                          prefix,
                                                       std::unordered_map<std::string, T*>& lora_weights,
                                                       float                                lora_alpha,
                                                       bool                                 from_outside)
{
...
template class GLVCrossAttnUpBlock2dWeight<float>;
template class GLVCrossAttnUpBlock2dWeight<half>;
}  // namespace lyradiff