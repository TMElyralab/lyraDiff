...
// #include "src/lyradiff/utils/assert.h"
...
public:
    ...
    virtual uint64_t hashID(const KernelMeta& kernelMeta) const
    {
        ...
        return hashID(kernelMeta.mS,
                      ...
                      kernelMeta.mFP32Accumulation,
                      kernelMeta.mFlashAttention,
                      kernelMeta.mAttentionMaskType,
                      kernelMeta.mTiled);
    }

    virtual void
    run(Fused_multihead_attention_params_v2& params, Launch_params& launch_params, cudaStream_t stream) const
    {
        ...
        const auto findIter = mFunctions.find(hashID(launch_params.kernel_s,
                                                     params.d,
                                                     launch_params.interleaved,
                                                     forceUnroll,
                                                     launch_params.force_fp32_acc,
                                                     launch_params.flash_attention,
                                                     static_cast<int>(launch_params.attention_mask_type),
                                                     launch_params.granular_tiling));
        LYRA_CHECK_WITH_INFO(findIter != mFunctions.end(), "FMHA kernels are not found");
        ...
        else {  // forceunroll = true for flash attention kernels
            int unroll = kernelMeta.mS / kernelMeta.mUnrollStep;
            LYRA_CHECK_WITH_INFO(kernelMeta.mS == kernelMeta.mUnrollStep * unroll, "Wrong launching sequence length");
