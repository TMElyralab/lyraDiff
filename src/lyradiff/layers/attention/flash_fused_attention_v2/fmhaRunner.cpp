...
public:
    ...
    {
        LYRA_CHECK_WITH_INFO((sm == kSM_80 || sm == kSM_86 || sm == kSM_89 || sm == kSM_90),
                             "Unsupported architecture");
        LYRA_CHECK_WITH_INFO((mDataType == DATA_TYPE_FP16 || mDataType == DATA_TYPE_BF16), "Unsupported data type");
    ...
    void set_tma_descriptors()
    {
        ...
        const cudaTmaDescSwizzle swizzle_mode =
        ...
        // Q: STEP_Q
        box_size[3] = q_step;
        qkv_tma_descriptor.set_tma_desctriptor(qkv_ptr,
                                               cudaTmaDescFormat::F16_RN,
...
FusedMHARunnerV2::FusedMHARunnerV2(const Data_type data_type,
                                   const int       numHeads,
                                   const int       headSize,
                                   const float     qScaling):
    pimpl(new mhaImpl(data_type, numHeads, headSize, qScaling, lyradiff::getSMVersion()))
{
}

FusedMHARunnerV2::~FusedMHARunnerV2() = default;

void FusedMHARunnerV2::setup(const int  b,
                             const int  s,
                             const int  total_seqlen,
                             const bool has_alibi,
                             const bool scale_alibi,
...
void FusedMHARunnerV2::setup_flags(const bool force_fp32_acc,
