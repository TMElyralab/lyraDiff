...
__global__ void wmma_attention_long_kernel(const half2*  qkv,
...
#pragma unroll
        for (int i = 0; i < n; i++) {
            ...
            if (to_id[i] < seq_len) {
                float mask = (float)__ldg(&attention_mask[(batch_seq_block_offset + from_id) * seq_len + to_id[i]]);
...
#pragma unroll
    ...
    for (int seq_id = warpId; seq_id < seq_len; seq_id += warpNums) {
        int pos                              = (batch_seq_offset + seq_id) * (half_hidden_dim * 3) + thread_offset;
        ((__half2*)(s_kv[seq_id]))[warp_tid] = __hadd2(__ldg(&qkv[pos + half_hidden_dim * 2]), v_bias);
    }
