...
public:
    ...
    CUTLASS_HOST_DEVICE
    AttentionGemmProblemSize(Params const& params): seqlen_offsets_(params.seqlen_offsets), seqlen_(params.seqlen) {}
    CUTLASS_DEVICE
    cutlass::gemm::GemmCoord operator()(int problem_idx) const
    {
        int valid_seqlen     = seqlen_offsets_ == nullptr ?
                                   seqlen_ :
                                   __ldg(&seqlen_offsets_[problem_idx + 1]) - __ldg(&seqlen_offsets_[problem_idx]);
...
public:
    ...
    {
        ptr_ = params.ptr + (problem_idx * head_num_ + batch_idx) * row_num_ * threadblock_num_in_row_ * 2;
    }
    ...
    CUTLASS_HOST_DEVICE typename Iterator::Fragment
    operator()(typename Iterator::Fragment const& frag, SourceFragment const& source, Iterator const& iterator) const
    {
        using ThreadMap = typename Iterator::ThreadMap;
        static_assert(ThreadMap::kElementsPerAccess == kCount, "kElementsPerAccess must match thread map");
        static_assert(Iterator::Fragment::kElements == SourceFragment::kElements, "kElements of source must match");
        ...
        cutlass::NumericArrayConverter<ElementCompute, ElementAccumulator, kCountRow, Round> accumulator_converter;
        cutlass::NumericArrayConverter<ElementCompute, ElementOutput, kCountRow, Round>      source_converter;
        cutlass::NumericArrayConverter<ElementOutput, ElementCompute, kCountRow, Round>      output_converter;
        ...
        for (int row = 0; row < ThreadMap::Iterations::kRow; ++row) {
            ...
            ReductionFragmentinTensorCoreShape<ComputeFragmentRow, cutlass::maximum<ElementCompute>, kAccessWidth>
                max_op;
            ...
            ReductionFragmentinTensorCoreShape<ComputeFragmentRow, cutlass::plus<ElementCompute>, kAccessWidth> sum_op;
            ...
            cutlass::arch::global_store<StoreType, sizeof(StoreType)>(
                value,
                reinterpret_cast<void*>(row_ptr + threadblock_column_idx * 2),
...
__global__ void softmax_reduction_kernel(
    const T* buf, const int* seqlen_offsets, T* out, const int column, const bool is_remove_padding)
{
    ...
    const int valid_seqlen =
        is_remove_padding ? (__ldg(&seqlen_offsets[batch_idx + 1]) - __ldg(&seqlen_offsets[batch_idx])) : row_num;
...
template<typename T, int ThreadblockN>
void softmax_reduction_kernel_launcher(const T*     buf,
...
public:
    CUTLASS_HOST_DEVICE
    SoftmaxPartialPrologueOp(Params const&        params,
                             int                  problem_idx,
                             int                  batch_idx,
                             cutlass::MatrixCoord extent,
                             cutlass::MatrixCoord warp_offset):
        head_num_(params.head_num),
        row_num_(params.row_num),
    ...
    template<typename WarpLoadIterator>
    CUTLASS_HOST_DEVICE typename WarpLoadIterator::Fragment
    operator()(typename WarpLoadIterator::Fragment const&         mma_frag,
               typename Detail<WarpLoadIterator>::Fragment const& frag) const
    {
        ...
        static_assert(FragmentMma::kElements == kAccessSize * IteratorDetail::kRows, "FragmentMma size not valid");
        ...
        if (is_residual_tile_ && residual && kgroup_index_ * IteratorDetail::InstructionShape::kColumn >= residual) {
            ret.clear();
            return ret;
        }
...
public:
    ...
    CUTLASS_HOST_DEVICE
    FragmentAccumulator operator()(FragmentAccumulator const& accumulator, FragmentOutput const& source) const
    {
        ...
        FragmentOutput source_mask = mask_converter(source, ElementOutput(10000.0), ElementOutput(-10000.0));
        ...
        ReductionFragmentinTensorCoreShape<ComputeFragment, cutlass::maximum<ElementCompute>, kAccessWidth> max_op;
        FragmentReductionAccumulator max_val = max_op(intermediate);
        ...
        ReductionFragmentinTensorCoreShape<ComputeFragment, cutlass::plus<ElementCompute>, kAccessWidth> sum_op;
        FragmentReductionAccumulator                sum = sum_op(intermediate);
...
public:
    ...
    struct Detail {
        ...
        using WarpPitchLinearShape =
            typename std::conditional<kIsOperandA,
                                      cutlass::PitchLinearShape<WarpShape::kColumn, WarpShape::kRow>,
        ...
        using InstructionPitchLinearShape = typename std::conditional<
            kIsOperandA,
            cutlass::PitchLinearShape<InstructionShape::kColumn, InstructionShape::kRow>,
...
public:
    CUTLASS_HOST_DEVICE QKBiasAddOp(Params const&        params,
                                    int                  problem_idx,
                                    int                  batch_idx,
                                    cutlass::MatrixCoord extent,
        ...
        valid_seqlen_(params.seqlen_offsets == nullptr ?
                          params.seqlen :
                          __ldg(&params.seqlen_offsets[problem_idx + 1]) - __ldg(&params.seqlen_offsets[problem_idx])),
    ...
    CUTLASS_DEVICE void set_kgroup_index(int k_group)
    {
        ...
        if constexpr (kRoformer) {
            int warp_contiguous_offset = this->kgroup_index_warp_ * IteratorDetail::WarpPitchLinearShape::kContiguous;
            int tile_contigous_offset  = this->kgroup_index_ * IteratorDetail::InstructionPitchLinearShape::kContiguous;
            int thread_congituous_offset = IteratorDetail::calc_thread_contiguous_offset(threadIdx.x % 32);
            int contiguous_offset        = warp_contiguous_offset + tile_contigous_offset + thread_congituous_offset;
            this->position_denominator_  = __powf(10000.0f, __fdividef(contiguous_offset & 0xFFFE, kSizePerHead));
        }
    }
    ...
    CUTLASS_DEVICE void load(typename Detail<WarpLoadIterator>::Fragment& frag)
    {
        ...
        const int thread_contiguous_offset = IteratorDetail::calc_thread_contiguous_offset(threadIdx.x % 32);
        ...
        for (int i = 0; i < IteratorDetail::kContiguousCount; ++i) {
            const int k_group_tile_offset = warp_tile_contigous_offset + i * IteratorDetail::kContiguousDelta;
    ...
    /// transform mma frag
    template<typename WarpLoadIterator>
    CUTLASS_DEVICE typename WarpLoadIterator::Fragment
    operator()(typename WarpLoadIterator::Fragment const&         mma_frag,
               typename Detail<WarpLoadIterator>::Fragment const& frag) const
    {
        using IteratorDetail = Detail<WarpLoadIterator>;
        using FragmentMma    = typename WarpLoadIterator::Fragment;
        FragmentMma ret;
        static_assert(IteratorDetail::kStridedCount * IteratorDetail::kAccessSize == FragmentMma::kElements,
        ...
        int valid_threshold =
            (valid_seqlen_ - strided_offset + (IteratorDetail::kStridedDelta - 1)) / IteratorDetail::kStridedDelta;
...
public:
    ...
    struct Detail {
        using WarpShape        = typename WarpLoadIterator::Shape;
        using InstructionShape = typename WarpLoadIterator::InstructionShape;
        static_assert(WarpLoadIterator::kOperand == cutlass::gemm::Operand::kB, "Value bias must be B operand");
        static_assert(InstructionShape::kColumn == 8, "Only support 8 Instruction Column");
...
public:
    CUTLASS_HOST_DEVICE
    ValueBiasAddOp(Params const&        params,
                   int                  problem_idx,
                   int                  batch_idx,
                   cutlass::MatrixCoord extent,
                   cutlass::MatrixCoord warp_offsets):
               ...
               typename Detail<WarpLoadIterator>::Fragment const& frag) const
    {
        ...
        if (this->kgroup_index_warp_ == 0) {
            valid_threshold = std::min(residual, valid_seqlen_) - instruction_row_offset - thread_row_offset;
        }
        else {
            int threadblock_row_offset = residual + (this->kgroup_index_warp_ - 1) * IteratorDetail::WarpShape::kRow;
            valid_threshold = valid_seqlen_ - threadblock_row_offset - instruction_row_offset - thread_row_offset;
        }
        ...
        for (int i = 0; i < IteratorDetail::kAccessSize; ++i) {
            if (i < valid_threshold) {
                ...
                for (int j = 0; j < IteratorDetail::kIterations; ++j) {
                    ret[j * IteratorDetail::kAccessSize + i] = mma_frag[j * IteratorDetail::kAccessSize + i] + frag[j];
                }
            }
            else {
                CUTLASS_PRAGMA_UNROLL
