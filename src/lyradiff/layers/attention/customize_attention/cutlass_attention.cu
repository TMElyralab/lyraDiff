...
#include "cutlass_attention_defs.h"
...
void gemm0_and_softmax_reduce_kernel_launcher(DataType*    query,
                                              DataType*    key,
                                              DataType*    atten_mask,
                                              DataType*    qk_output,
                                              float*       partial_softmax_buf,
                                              float*       softmax_reduced_buf,
                                              int*         seqlen_offsets,
                                              ...
                                              cudaStream_t stream)
{
    ...
    typename GemmKernel::ParamsDef::ProblemSizeOperator::Params problem_size_op{seqlen_offsets, seq_len};
    typename GemmKernel::ParamsDef::BatchCountOperator::Params  batch_count_op{head_num};
    typename AttentionTensorParamGeneratorOp<ElementA>::Params  param_A_op{reinterpret_cast<ElementA*>(query),
                                                                          size_per_head,
                                                                          batch_size * seq_len * size_per_head,
                                                                          seq_len * size_per_head,
                                                                          nullptr};
    typename AttentionTensorParamGeneratorOp<ElementB>::Params  param_B_op{reinterpret_cast<ElementB*>(key),
                                                                          size_per_head,
                                                                          batch_size * seq_len * size_per_head,
                                                                          seq_len * size_per_head,
                                                                          nullptr};
    ...
    typename AttentionTensorParamGeneratorOp<ElementD>::Params param_D_op{
        reinterpret_cast<ElementD*>(qk_output), seq_len, seq_len * seq_len, seq_len * seq_len * head_num, nullptr};
    ...
    auto args = typename Gemm::Arguments(cutlass::gemm::GemmUniversalMode::kBatched,
                                         problem_count,
                                         cta_count,
                                         problem_size_op,
                                         batch_count_op,
    ...
    softmax_reduction_kernel_launcher<float, Shape::kN>(partial_softmax_buf,
                                                        seqlen_offsets,
                                                        softmax_reduced_buf,
                                                        batch_size,
                                                        head_num,
...
void gemm1_kernel_launcher(DataType*    qk_output,
                           DataType*    value,
                           DataType*    attention_output,
                           float*       softmax_reduced_buf,
                           int const*   seqlen_offsets,
                           const int    batch_size,
                           const int    seq_len,
                           const int    head_num,
                           const int    size_per_head,
                           ...
                           cudaStream_t stream)
{
    ...
    typename AttentionTensorParamGeneratorOp<ElementA>::Params param_A_op{
        reinterpret_cast<ElementA*>(qk_output), seq_len, seq_len * seq_len, seq_len * seq_len * head_num, nullptr};

    typename AttentionTensorParamGeneratorOp<ElementB>::Params param_B_op{reinterpret_cast<ElementB*>(value),
                                                                          size_per_head,
                                                                          batch_size * seq_len * size_per_head,
                                                                          seq_len * size_per_head,
                                                                          nullptr};
    typename AttentionTensorParamGeneratorOp<ElementC>::Params param_C_op{nullptr, 0, 0, 0};
    typename AttentionTensorParamGeneratorOp<ElementD>::Params param_D_op{reinterpret_cast<ElementD*>(attention_output),
                                                                          hidden_dim,
                                                                          size_per_head,
                                                                          is_remove_padding ? hidden_dim :
                                                                                              seq_len * hidden_dim,
                                                                          seqlen_offsets};

    const int                                           problem_count = batch_size;
    typename GemmKernel::PrologueDefA::Operator::Params prologue_A{head_num, seq_len, softmax_reduced_buf};
    auto args = typename Gemm::Arguments(cutlass::gemm::GemmUniversalMode::kBatched,
                                         problem_count,
                                         cta_count,
                                         problem_size_op,
...
void CutlassAttention<OpType>::infer_impl(AttentionInferParam* infer_param)
{
    ...
    auto softmax_reduced_buf = (float*)((uint8_t*)partial_softmax_buf + this->buf_sizes_.partial_softmax);
    ...
    gemm0_and_softmax_reduce_kernel_launcher<typename CutlassAttentionCore::Gemm0, DataType_, kMaxThreadblockNumInRow>(
        query,
        key,
        atten_mask_noconst,
        qk_output,
        ...
        size_per_head,
        this->param_.tao,
        false,  // this->is_remove_padding_,
        this->multi_processor_count_,
        stream);

    gemm1_kernel_launcher<typename CutlassAttentionCore::Gemm1, DataType_>(qk_output,
                                                                           value,
                                                                           infer_param->attention_output,
                                                                           softmax_reduced_buf,
                                                                           seqlen_offsets,
                                                                           batch_size,
                                                                           seq_len,
...
#define ATTENTION_CORE_DEF(ARCH) using CutlassAttentionCore = CutlassAttentionCore<SeqLen, SizePerHead, ARCH, 2>;

#define INFER_WITH_KNOWN_ARCH(ARCH)                                                                                    \
    ATTENTION_CORE_DEF(ARCH)                                                                                           \
...
void CutlassAttention<OpType>::infer(AttentionInferParam infer_param)
{
    ...
    static_assert((__CUDACC_VER_MAJOR__ * 100 + __CUDACC_VER_MINOR__) >= 1106, "NVCC must be newer than 11.6");
