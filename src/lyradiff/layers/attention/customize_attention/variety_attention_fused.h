...
__global__ void variety_wmma_attention_kernel(const half2*  query,
                                              const half2*  key,
                                              const half2*  value,
                                              const __half* attention_mask,
                                              const __half* qk_buf,
                                              const __half* attention_bias,
...
__global__ void variety_wmma_attention_long_kernel(const half2*  query,
                                                   const half2*  key,
                                                   const half2*  value,
                                                   const __half* attention_mask,
                                                   const __half* qk_buf,
                                                   ...
                                                   const float   scale)
{
    ...
    __half(*s_logits)[max_seq_len + SKEW_HALF] =
        (__half(*)[max_seq_len + SKEW_HALF])(base + (split_seq_len + max_seq_len) * (size_per_head + SKEW_HALF));
...
#pragma unroll
        for (int i = 0; i < n; i++) {
            ...
            if (to_id[i] < seq_len) {
                ...
                float     qk = qk_buf ? (float)(qk_buf[(gemm_offset + split_seq_offset) * seq_len + to_id[i]]) : 0.0f;
                float     bias =
                    attention_bias ?
                            (float)(attention_bias[(blockIdx.x * seq_len + split_seq_offset) * seq_len + to_id[i]]) :
                            0.0f;

                float mask = (float)__ldg(&attention_mask[(batch_seq_offset + split_seq_offset) * seq_len + to_id[i]]);
                mask       = (1.0f - mask) * (-10000.0f);
                logits[i]  = (float)(s_logits[from_id][to_id[i]]) * scale + qk + bias + mask;
            }
            max_val = max(max_val, logits[i]);
...
#define X_WMMA_ATTENTION_LONG(SEQ_LEN, SIZE_PER_HEAD, SPLIT_LEN)                                                       \
    shared_memory_size =                                                                                               \
        ((SEQ_LEN + SPLIT_LEN) * (SIZE_PER_HEAD + SKEW_HALF) + SPLIT_LEN * (SEQ_LEN + SKEW_HALF)) * 2;                 \
...
void fused_infer(const half2*  query,
                 const half2*  key,
                 const half2*  value,
                 const __half* atten_mask,
                 const __half* qk_buf,
                 const __half* attention_bias,
                 __half*       attention_output,
                 const int     head_num,
...
void fused_long_infer(const half2*  query,
                      const half2*  key,
                      const half2*  value,
                      const __half* atten_mask,
                      const __half* qk_buf,
...
template<typename T>
void variety_attention_fused_infer(const T*     q,
                                   const T*     k,
                                   const T*     v,
                                   const T*     atten_mask,
                                   const T*     qk_buf,
