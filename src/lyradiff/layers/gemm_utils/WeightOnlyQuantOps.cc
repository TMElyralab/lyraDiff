...
static auto symmetric_quantize_last_axis_of_batched_matrix =
    torch::RegisterOperators("lyrallms::symmetric_quantize_last_axis_of_batched_matrix",
                             &torch_ext::symmetric_quantize_last_axis_of_batched_matrix);

static auto preprocess_weights_for_mixed_gemm = torch::RegisterOperators("lyrallms::preprocess_weights_for_mixed_gemm",
                                                                         &torch_ext::preprocess_weights_for_mixed_gemm);

static auto unpack_int4_packed_tensor_to_int8 = torch::RegisterOperators("lyrallms::unpack_int4_packed_tensor_to_int8",
                                                                         &torch_ext::unpack_int4_packed_tensor_to_int8);

static auto pack_int8_tensor_to_packed_int4 =
    torch::RegisterOperators("lyrallms::pack_int8_tensor_to_packed_int4", &torch_ext::pack_int8_tensor_to_packed_int4);

// Utility methods exposed purely for unit tests in torch.
static auto _symmetric_quantize_last_axis_of_batched_matrix =
    torch::RegisterOperators("lyrallms::_symmetric_quantize_last_axis_of_batched_matrix",
                             &torch_ext::_symmetric_quantize_last_axis_of_batched_matrix);

static auto add_bias_and_interleave_int4s =
    torch::RegisterOperators("lyrallms::_add_bias_and_interleave_int4s", &torch_ext::add_bias_and_interleave_int4s);

static auto add_bias_and_interleave_int8s =
    torch::RegisterOperators("lyrallms::_add_bias_and_interleave_int8s", &torch_ext::add_bias_and_interleave_int8s);

static auto permute_B_rows_for_mixed_gemm =
    torch::RegisterOperators("lyrallms::_permute_B_rows_for_mixed_gemm", &torch_ext::permute_B_rows_for_mixed_gemm);

static auto subbyte_transpose = torch::RegisterOperators("lyrallms::_subbyte_transpose", &torch_ext::subbyte_transpose);