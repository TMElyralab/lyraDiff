#include "FluxAttentionFP8Processor.h"
...
template<typename T>
FluxAttentionFP8Processor<T>::FluxAttentionFP8Processor(size_t           embedding_dim,
...
template<typename T>
FluxAttentionFP8Processor<T>::FluxAttentionFP8Processor(FluxAttentionFP8Processor<T> const& other):
...
template<typename T>
void FluxAttentionFP8Processor<T>::allocateBuffer()
{
    LYRA_CHECK_WITH_INFO(
        false,
        "FluxAttentionFP8Processor::allocateBuffer() is deprecated. Use `allocateBuffer(size_t batch_size, ...)` instead");
}

template<typename T>
void FluxAttentionFP8Processor<T>::allocateBuffer(size_t batch_size, size_t seq_len, size_t encoder_seq_len)
{
...
template<typename T>
void FluxAttentionFP8Processor<T>::freeBuffer()
{
...
template<typename T>
void FluxAttentionFP8Processor<T>::forward(const TensorMap*                          output_tensors,
                                           ...
                                           const FluxAttentionFP8ProcessorWeight<T>* weights)
{
    ...
    invokeCudaD2DScaleCpyConvert(fp8_buffer,
                                 ...
                                 weights->encoder_to_qkv_input_scale,
                                 true,
                                 batch_size * encoder_seq_len * this->embedding_dim_,
                                 this->stream_);

    m_1 = batch_size * encoder_seq_len;
    n_1 = this->embedding_dim_ * 3;
    k_1 = this->embedding_dim_;
    ...
    this->cublas_wrapper_->GemmWithResidualAndBias(CUBLAS_OP_T,
                                                   ...
                                                   k_1,                                   // LDA
                                                   weights->encoder_to_qkv_weight_scale,  // weight scale
                                                   fp8_buffer,                            // B
                                                   k_1,                                   // LDB
                                                   weights->encoder_to_qkv_input_scale,   // input scale
    ...
    Tensor encoder_qkv = Tensor(MEMORY_GPU,
                                input_tensor.type,
                                {batch_size, encoder_seq_len, 3, this->embedding_head_num_, this->embedding_head_dim_},
                                encoder_qkv_buffer);
    ...
    this->cublas_wrapper_->GemmWithResidualAndBias(CUBLAS_OP_T,
                                                   ...
                                                   k_1,                           // LDA
                                                   weights->to_out_weight_scale,  // weight scale
                                                   fp8_buffer,                    // B
                                                   k_1,                           // LDB
                                                   weights->to_out_input_scale,   // input scale
    ...
    invokeCudaD2DScaleCpyConvert(fp8_buffer,
                                 encoder_hidden_ptr,
                                 weights->encoder_to_out_input_scale,
                                 true,
                                 batch_size * encoder_seq_len * this->embedding_dim_,
                                 this->stream_);
    ...
    this->cublas_wrapper_->GemmWithResidualAndBias(CUBLAS_OP_T,
                                                   ...
                                                   k_1,                                   // LDA
                                                   weights->encoder_to_out_weight_scale,  // weight scale
                                                   fp8_buffer,                            // B
                                                   k_1,                                   // LDB
                                                   weights->encoder_to_out_input_scale,   // input scale
...
template<typename T>
FluxAttentionFP8Processor<T>::~FluxAttentionFP8Processor()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    freeBuffer();
...
template class FluxAttentionFP8Processor<float>;
template class FluxAttentionFP8Processor<half>;
#ifdef ENABLE_BF16
template class FluxAttentionFP8Processor<__nv_bfloat16>;
#endif
}  // namespace lyradiff