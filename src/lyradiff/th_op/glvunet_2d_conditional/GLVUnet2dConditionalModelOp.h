...
#include "src/lyradiff/models/glv_unet_2d_conditional_model/GLVUnet2dConditionalModel.h"
...
template<typename T>
class lyradiffGLVUnet2dConditionalModel: public IFUnet {
...
private:
    ...
    GLVUnet2dConditionalModel<T>*       model_;
    GLVUnet2dConditionalModelWeight<T>* model_weight_;

    std::unordered_map<std::string, GLVControlNetModelWeight<T>*> loaded_controlnet_weights;

    std::vector<GLVControlNetModelWeight<T>*> cache_controlnet_weights;
    ...
    int64_t max_controlnet_count_ = 1;
...
public:
    lyradiffGLVUnet2dConditionalModel(const std::string inference_dtype,
                                      ...
                                      const int64_t     output_channels)
    {
        ...
        model_ = new GLVUnet2dConditionalModel<T>(
            cudnn_handle_, stream_, cublas_wrapper_, allocator_, false, false, false, input_channels, output_channels);

        model_weight_ = new GLVUnet2dConditionalModelWeight<T>(input_channels, output_channels);
    }

    ~lyradiffGLVUnet2dConditionalModel() override
    {
        delete model_;
                       ...
                       th::optional<th::Dict<std::string, double>>     scale_params)
    {
        ...
        std::vector<GLVControlNetModelWeight<T>*> input_controlnet_weights;
                               ...
                               const std::string model_dtype)
    {
        ...
        if (cache_controlnet_weights.size() > 0) {
            GLVControlNetModelWeight<T>* cur_controlnet = cache_controlnet_weights[cache_controlnet_weights.size() - 1];
            ...
            GLVControlNetModelWeight<T>* cur_controlnet = new GLVControlNetModelWeight<T>();
            cur_controlnet->loadWeights(model_path, model_file_type);
            loaded_controlnet_weights[controlnet_name] = cur_controlnet;
        }

        model_weight_->loadProjectWeights(model_path, model_file_type);
        lock_->unlock();
    }
                                               ...
                                               const std::string                 device)
    {
        ...
        model_weight_->loadProjectWeightsFromCache("", tmp_weights, memcpyKind);
    ...
    void clean_controlnet_cache()
    {
        ...
        cache_controlnet_weights = std::vector<GLVControlNetModelWeight<T>*>();
        lock_->unlock();
    }
};

class GLVUnet2dConditionalModelOp: public th::jit::CustomClassHolder {
public:
    GLVUnet2dConditionalModelOp(const std::string inference_dtype,
                                const int64_t     input_channels,
                                const int64_t     output_channels);
    ~GLVUnet2dConditionalModelOp();
