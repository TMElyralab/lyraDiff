#include "FluxTransformerBlockOp.h"

namespace th = torch;
namespace torch_ext {

FluxTransformerBlockOp::FluxTransformerBlockOp(const std::string inference_dtype,
    ...
    inference_dtype_(inference_dtype)
{
    if (inference_dtype_ == "fp32") {
        model_ = new LyraDiffFluxTransformerBlock<float>(
            embedding_dim, embedding_head_num, embedding_head_dim, mlp_scale, quant_level);
    }
    else if (inference_dtype_ == "fp16") {
        model_ = new LyraDiffFluxTransformerBlock<half>(
            embedding_dim, embedding_head_num, embedding_head_dim, mlp_scale, quant_level);
    }
#ifdef ENABLE_BF16
    else if (inference_dtype_ == "bf16") {
        model_ = new LyraDiffFluxTransformerBlock<__nv_bfloat16>(
            embedding_dim, embedding_head_num, embedding_head_dim, mlp_scale, quant_level);
    }
#endif
    else {
...
FluxTransformerBlockOp::~FluxTransformerBlockOp()
{
    delete model_;
}

std::vector<th::Tensor> FluxTransformerBlockOp::forward(th::Tensor hidden_states,
...
#endif
    return model_->forward(hidden_states, encoder_hidden_states, rope_emb, temb_emb);
}

void FluxTransformerBlockOp::initialize(const c10::intrusive_ptr<torch_ext::LyraDiffCommonContext>& lyradiff_context)
{
    model_->initialize(lyradiff_context);
}

void FluxTransformerBlockOp::reload_model_from_bin(const std::string model_path, const std::string model_dtype)
{
    model_->reload_model_from_bin(model_path, model_dtype);
}

void FluxTransformerBlockOp::reload_model_from_state_dict(const std::string                 prefix,
...
void FluxTransformerBlockOp::clear_lora()
{
    model_->clear_lora();
}

void FluxTransformerBlockOp::load_lora_by_name(std::vector<std::string> lora_name,
