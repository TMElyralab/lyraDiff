...
template<typename T>
class LyraDiffUnet2dConditionalModel: public IFUnet {
...
private:
    ...
    Unet2dConditionalModel<T>*       model_;
    Unet2dConditionalModelWeight<T>* model_weight_;
    size_t                           input_channels_;
    size_t                           output_channels_;

    std::unordered_map<std::string, ControlNetModelWeight<T>*> loaded_controlnet_weights;

    std::vector<ControlNetModelWeight<T>*> cache_controlnet_weights;
...
public:
    LyraDiffUnet2dConditionalModel(const int64_t      max_controlnet_count,
                                   const std::string  inference_dtype,
                                   const int64_t      input_channels,
                                   const int64_t      output_channels,
                                   const int64_t      quant_level,
                                   const std::string& sd_ver):
        input_channels_(input_channels), output_channels_(output_channels), quant_level_(quant_level)
    {
        ...
        model_ = new Unet2dConditionalModel<T>(max_controlnet_count,
                                               cudnn_handle_,
                                               stream_,
                                               cublas_wrapper_,
                                               allocator_,
        ...
        model_weight_ =
            new Unet2dConditionalModelWeight<T>(input_channels, output_channels, quant_level_enum_, allocator_, sd_ver);
    }

    ~LyraDiffUnet2dConditionalModel() override
    {
        delete model_;
                       ...
                       th::optional<th::Dict<std::string, double>>     scale_params) override
    {
        ...
        std::vector<ControlNetModelWeight<T>*> input_controlnet_weights;
        ...
        if (controlnet_names.has_value() && controlnet_conds.has_value() && controlnet_scales.has_value()) {
            if (controlnet_names.value().size() != controlnet_conds.value().size()) {
                throw "controlnet_names and controlnet_conds has different size";
            }

            if (controlnet_names.value().size() != controlnet_scales.value().size()) {
                throw "controlnet_names and controlnet_scales has different size";
            }
        ...
        // cout << "input_conds.size() " << input_conds.size() << endl;
        // cout << "input_controlnet_scales.size() " << input_controlnet_scales.size() << endl;
        // cout << "input_controlnet_weights.size() " << input_controlnet_weights.size() << endl;
        lock_->lock();
    ...
    th::Tensor unet_forward(th::Tensor                                      hidden_states,
                            ...
                            th::optional<vector<th::Tensor>>                controlnet_results = th::nullopt,
                            th::optional<th::Dict<std::string, th::Tensor>> map_extra_tensors  = th::nullopt,
                            th::optional<th::Dict<std::string, double>>     scale_params       = th::nullopt) override
    {
        size_t                        batch_size = hidden_states.size(0);
        size_t                        height     = hidden_states.size(1);
        size_t                        width      = hidden_states.size(2);
        ...
        // printf("map_extra_tensors size: %d, context size: %d\n", map_extra_tensors.value().size(),
        // context.keys().size());
        for (auto& key : context.keys()) {
            weight_loader_manager_glob->map_lora_container["unet"].set_map_de_mods(
                key, (void*)(context.at(key).getPtr<T>()));
        }
        ...
        // cout << "input_controlnet_weights.size() " << input_controlnet_weights.size() << endl;
        lock_->lock();
        model_->unet_forward(&output_tensor, &input_tensor, timestep, model_weight_, controlnet_results_lyra_tensors);
        cudaDeviceSynchronize();
        lock_->unlock();
                                          ...
                                          th::Dict<std::string, th::Tensor> weights_beta)
    {
        ...
        auto                        load_func = [&](th::Dict<std::string, th::Tensor>& weights, bool is_alpha) {
            ...
            model_weight_->loadS3DiffLoraFromStateDict(tmp_weights, is_alpha);
        };
                               ...
                               const std::string model_dtype)
    {
        ...
        if (cache_controlnet_weights.size() > 0) {
            ControlNetModelWeight<T>* cur_controlnet = cache_controlnet_weights[cache_controlnet_weights.size() - 1];
            ...
            ControlNetModelWeight<T>* cur_controlnet = new ControlNetModelWeight<T>();
            cur_controlnet->loadWeights(model_path, model_file_type);
            loaded_controlnet_weights[controlnet_name] = cur_controlnet;
        }
        lock_->unlock();
    ...
    void clean_controlnet_cache()
    {
        ...
        cache_controlnet_weights = std::vector<ControlNetModelWeight<T>*>();
        lock_->unlock();
    }
...
class Unet2dConditionalModelOp: public th::jit::CustomClassHolder {
public:
    Unet2dConditionalModelOp(const int64_t             max_controlnet_count,
                             const std::string         inference_dtype,
