...
template<typename T>
FluxControlnetModel<T>::FluxControlnetModel(cudaStream_t        stream,
    ...
    guidance_embeds_(guidance_embeds)
{
    ...
    if (quant_level != LyraQuantType::NONE) {
...
template<typename T>
FluxControlnetModel<T>::FluxControlnetModel(FluxControlnetModel<T> const& other):
...
template<typename T>
void FluxControlnetModel<T>::allocateBuffer()
{
    LYRA_CHECK_WITH_INFO(
        false,
        "FluxControlnetModel::allocateBuffer() is deprecated. Use `allocateBuffer(size_t batch_size, ...)` instead");
}

template<typename T>
void FluxControlnetModel<T>::allocateBuffer(size_t batch_size, size_t seq_len, size_t encoder_seq_len)
{
...
template<typename T>
void FluxControlnetModel<T>::freeBuffer()
{
...
template<typename T>
void FluxControlnetModel<T>::updateConfig(const size_t num_layers, const size_t num_single_layers)
{
    num_layers_        = num_layers;
    num_single_layers_ = num_single_layers;
}
...
template<typename T>
void FluxControlnetModel<T>::controlnet_forward(std::vector<Tensor>&                output_tensors,
                                                ...
                                                const FluxControlnetModelWeight<T>* weights)
{
    ...
    Tensor controlnet_condition  = input_tensors->at("controlnet_condition");

    LYRA_CHECK_WITH_INFO(single_output_tensors.size() == num_single_layers_,
                         "single_output_tensors length has to be the same as num_single_layers config");

    LYRA_CHECK_WITH_INFO(output_tensors.size() == num_layers_,
                         "output_tensors length has to be the same as num_layers config");
    ...
    cublas_wrapper_->GemmWithResidualAndBias(CUBLAS_OP_T,
                                             ...
                                             m_1,                                    // n
                                             k_1,                                    // k
                                             weights->controlnet_x_embedder_weight,  // A
                                             k_1,                                    // LDA
                                             controlnet_condition.getPtr<T>(),       // B
                                             k_1,                                    // LDB
                                             hidden_buf_2,                           // C
                                             n_1,                                    // LDC
                                             weights->controlnet_x_embedder_bias,    // bias
                                             nullptr,                                // residual
                                             1.0f,                                   // alpha
                                             0.0f);                                  // beta
    ...
    for (int i = 0; i < num_layers_; i++) {
        ...
        cublas_wrapper_->GemmWithResidualAndBias(CUBLAS_OP_T,
                                                 ...
                                                 m_1,                            // n
                                                 k_1,                            // k
                                                 weights->control_weights[i],    // A
                                                 k_1,                            // LDA
                                                 hidden_output_ptr,              // B
                                                 k_1,                            // LDB
                                                 output_tensors[i].getPtr<T>(),  // C
                                                 n_1,                            // LDC
                                                 weights->control_bias[i],       // bias
                                                 nullptr,                        // residual
                                                 controlnet_scale,               // alpha
                                                 0.0f);                          // beta

        // hidden_output.saveNpy("transformer_" + std::to_string(i) + ".npy");
        // encoder_output.saveNpy("transformer_encoder_" + std::to_string(i) + ".npy");
    }
...
template<typename T>
FluxControlnetModel<T>::~FluxControlnetModel()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template class FluxControlnetModel<float>;
template class FluxControlnetModel<half>;
#ifdef ENABLE_BF16
template class FluxControlnetModel<__nv_bfloat16>;
#endif
}  // namespace lyradiff