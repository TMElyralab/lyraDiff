...
namespace lyradiff {
template<typename T>
class XLUnet2dConditionalModelWeight: public IFCBasicTransformerContainerWeight<T> {
private:
...
public:
    ...
    XLCrossAttnUpBlock2dWeight<T>*      cross_attn_up_block_2d_weight1;
    XLCrossAttnUpBlock2dWeight<T>*      cross_attn_up_block_2d_weight2;
    XLUpBlock2dWeight<T>*               up_block_2d_weight;

    XLUnet2dConditionalModelWeight(const size_t        input_channels     = 4,
                                   const size_t        output_channels    = 4,
                                   const bool          use_runtime_augemb = false,
                                   const LyraQuantType quant_level        = LyraQuantType::NONE,
                                   IAllocator*         allocator          = nullptr);

    ~XLUnet2dConditionalModelWeight();
