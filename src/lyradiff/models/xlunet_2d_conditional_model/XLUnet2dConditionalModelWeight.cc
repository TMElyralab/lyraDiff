#include "XLUnet2dConditionalModelWeight.h"
...
template<typename T>
XLUnet2dConditionalModelWeight<T>::XLUnet2dConditionalModelWeight(const size_t        input_channels,
    ...
    use_runtime_augemb_(use_runtime_augemb), input_channels_(input_channels), output_channels_(output_channels)
{
    ...
    if (use_runtime_augemb_) {
        texttime_embedding_weight = new TextTimeEmbeddingBlockWeight<T>(
            block_out_channels_[0], add_emb_input_dim_, temb_channels_, temb_channels_, quant_level, allocator);
    }
    else {
    ...
    down_block_2d_weight = new XLDownBlock2DWeight<T>(
        block_out_channels_[0], block_out_channels_[0], block_out_channels_[0], true, allocator);
    ...
    cross_attn_up_block_2d_weight1 = new XLCrossAttnUpBlock2dWeight<T>(block_out_channels_[1],
                                                                       block_out_channels_[2],
                                                                       block_out_channels_[2],
                                                                       temb_channels_,
                                                                       head_nums_[1],
    ...
    cross_attn_up_block_2d_weight2 = new XLCrossAttnUpBlock2dWeight<T>(block_out_channels_[0],
                                                                       block_out_channels_[1],
                                                                       block_out_channels_[2],
                                                                       temb_channels_,
                                                                       head_nums_[0],
    ...
    up_block_2d_weight = new XLUpBlock2dWeight<T>(
        block_out_channels_[0], block_out_channels_[0], block_out_channels_[1], norm_num_groups_, allocator);
...
template<typename T>
XLUnet2dConditionalModelWeight<T>::~XLUnet2dConditionalModelWeight()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void XLUnet2dConditionalModelWeight<T>::mallocWeights()
{
    if (!is_maintain_buffer) {
...
template<typename T>
void XLUnet2dConditionalModelWeight<T>::loadLoraFromWeight(std::string                          lora_path,
                                                           std::string                          prefix,
...
template<typename T>
void XLUnet2dConditionalModelWeight<T>::loadLoraFromCache(std::string                          prefix,
                                                          std::unordered_map<std::string, T*>& lora_weights,
                                                          float                                lora_alpha,
                                                          bool                                 from_outside)
{
...
template<typename T>
void XLUnet2dConditionalModelWeight<T>::loadWeights(const std::string prefix, FtCudaDataType model_file_type)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (!is_maintain_buffer) {
        mallocWeights();
...
template<typename T>
void XLUnet2dConditionalModelWeight<T>::loadWeightsFromCache(std::string                             prefix,
...
template class XLUnet2dConditionalModelWeight<float>;
template class XLUnet2dConditionalModelWeight<half>;
}  // namespace lyradiff