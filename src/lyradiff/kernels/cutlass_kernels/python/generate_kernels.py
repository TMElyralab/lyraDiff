...
class lyradiff_EpilogueTag(enum.Enum):
    epilogue_op_default = enum_auto()
    epilogue_op_bias = enum_auto()


EpiTagNames = {
    lyradiff_EpilogueTag.epilogue_op_default: "lc",  # linear combination
    lyradiff_EpilogueTag.epilogue_op_bias:
    "lc_bias"  # linear combination with bias addition
}

EpiTag = {
    lyradiff_EpilogueTag.epilogue_op_default:
    "lyradiff::cutlass_extensions::EpilogueOpDefault",
    lyradiff_EpilogueTag.epilogue_op_bias:
    "lyradiff::cutlass_extensions::EpilogueOpBias"
}


################################################################################
# Quantization Operation and string utils
class lyradiff_QuantOp(enum.Enum):
...
QuantOpNames = {
    lyradiff_QuantOp.per_column_scale_only: "cs",
    lyradiff_QuantOp.finegrained_scale_only: "fgs",
    lyradiff_QuantOp.finegrained_scale_and_zeros: "fgsz"
}

QuantOpTag = {
    lyradiff_QuantOp.per_column_scale_only:
    "cutlass::WeightOnlyQuantOp::PER_COLUMN_SCALE_ONLY",
    lyradiff_QuantOp.finegrained_scale_only:
    "cutlass::WeightOnlyQuantOp::FINEGRAINED_SCALE_ONLY",
    lyradiff_QuantOp.finegrained_scale_and_zeros:
...
################################################################################
# A data structure holding all info to instantiate gemm launchers in lyradiff.
class lyradiff_GemmLauncher:
...
{out_tag}*, int, int, int, const int, lyradiff::cutlass_extensions::CutlassGemmConfig, char*, size_t, cudaStream_t, int*
);
"""
...
def generate_sm90_operations():
    ...
    epi_tags = [lyradiff_EpilogueTag.epilogue_op_bias]
    ...
    for dtype_combo, quant_op, epi_tag, cta_shape_mn, cga_shape in partial_args:
        ...
        operation = lyradiff_GemmLauncher(GemmKind.Gemm, arch, *dtype_combo, quant_op, epi_tag, cta_shape_mnk, \
...
if __name__ == "__main__":
    ...
    hopper_inl = "src/lyradiff/kernels/cutlass_kernels/fpA_intB_gemm/launchers/fpA_intB_launcher_sm90.inl"
