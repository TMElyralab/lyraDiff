...
// #include "src/lyradiff/kernels/decoderMaskedMultiheadAttentionUtils.h"
...
void genericInt8GemmKernelLauncher(int8_t const*          A,
                                   int8_t const*          B,
                                   ...
                                   int*                   occupancy = nullptr)
{
    ...
    if (can_implement != cutlass::Status::kSuccess) {
        ...
        throw std::runtime_error("[lyradiff Error][int8gemm Runner] " + errMsg);
    }
...
struct dispatchStages {
    static void dispatch(int8_t const*          A,
                         int8_t const*          B,
                         tk::QuantMode          quantOption,
                         float const*           alphaCol,
                         float const*           alphaRow,
...
struct dispatchStages<T, arch, ThreadblockShape, WarpShape, 2> {
    ...
    {
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        genericInt8GemmKernelLauncher<T, arch, ThreadblockShape, WarpShape, 2>(A,
                                                                               B,
                                                                               quantOption,
                                                                               alphaCol,
                                                                               alphaRow,
...
struct dispatchStages<T,
                         ...
                         int*                   occupancy = nullptr)
    {
        ...
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        genericInt8GemmKernelLauncher<T, cutlass::arch::Sm80, ThreadblockShape, WarpShape, Stages>(A,
                                                                                                   B,
                                                                                                   quantOption,
                                                                                                   alphaCol,
                                                                                                   alphaRow,
...
template<typename T, typename arch, typename ThreadblockShape, typename WarpShape>
void dispatchGemmConfig(int8_t const*          A,
                        int8_t const*          B,
                        tk::QuantMode          quantOption,
                        float const*           alphaCol,
                        float const*           alphaRow,
...
void dispatchGemmToCutlass(int8_t const*          A,
                           int8_t const*          B,
                           tk::QuantMode          quantOption,
                           float const*           alphaCol,
                           float const*           alphaRow,
                           ...
                           int*                   occupancy = nullptr)
{
    ...
    switch (gemmConfig.tile_config) {
        ...
        case tkc::CutlassTileConfig::Undefined:
            throw std::runtime_error("[lyradiff Error][int8][dispatch_gemm_to_cutlass] gemm config undefined.");
            break;
        case tkc::CutlassTileConfig::ChooseWithHeuristic:
            throw std::runtime_error(
                "[lyradiff Error][int8][dispatch_gemm_to_cutlass] gemm config should have already been set by "
                "heuristic.");
            break;
        default:
            throw std::runtime_error(
                "[lyradiff Error][int8][dispatch_gemm_to_cutlass] Config is invalid for int8 GEMM.");
            break;
    }
}
...
template<typename T>
void CutlassInt8GemmRunner<T>::dispatchToArch(int8_t const*          A,
                                              int8_t const*          B,
                                              tk::QuantMode          quantOption,
                                              float const*           alphaCol,
                                              float const*           alphaRow,
                                              ...
                                              int*                   occupancy)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (mSm >= 70 && mSm < 72) {
        dispatchGemmToCutlass<T, cutlass::arch::Sm70>(A,
                                                      B,
                                                      quantOption,
                                                      alphaCol,
                                                      alphaRow,
        ...
        dispatchGemmToCutlass<T, cutlass::arch::Sm72>(A,
                                                      B,
                                                      quantOption,
                                                      alphaCol,
                                                      alphaRow,
        ...
        dispatchGemmToCutlass<T, cutlass::arch::Sm75>(A,
                                                      B,
                                                      quantOption,
                                                      alphaCol,
                                                      alphaRow,
        ...
        dispatchGemmToCutlass<T, cutlass::arch::Sm80>(A,
                                                      B,
                                                      quantOption,
                                                      alphaCol,
                                                      alphaRow,
...
template<typename T>
void CutlassInt8GemmRunner<T>::gemm(int8_t const*          A,
                                    int8_t const*          B,
                                    tk::QuantMode          quantOption,
                                    float const*           alphaCol,
                                    float const*           alphaRow,
...
std::vector<tkc::CutlassGemmConfig> CutlassInt8GemmRunner<T>::getConfigs() const
{
    ...
    std::vector<tkc::CutlassGemmConfig> candidateConfigs =
        get_candidate_configs(mSm, isWeightOnly, mSm <= 70, true, SPLIT_K_LIMIT);
