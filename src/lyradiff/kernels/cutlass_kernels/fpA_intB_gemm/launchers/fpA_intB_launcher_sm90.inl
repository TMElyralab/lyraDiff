...
#include "src/lyradiff/kernels/cutlass_kernels/cutlass_type_conversion.h"
#include "fpA_intB_launcher_sm90.h"
...
void sm90_generic_mixed_gemm_kernelLauncher(ActivationType const*  A,
                                            WeightType const*      B,
    ...
    if constexpr (cute::is_same_v<SupportedCtaShape, CTAShape> && cute::is_same_v<SupportedCgaShape, ClusterShape>) {
        ...
        static_assert(
            std::is_same_v<
                ...
                cutlass::
                    half_t> || std::is_same_v<CutlassActivationType, cutlass::bfloat16_t> || std::is_same_v<CutlassActivationType, cutlass::float_e4m3_t> || std::is_same_v<CutlassActivationType, cutlass::float_e5m2_t>,
            "Activation type must be bfloat16, half, FP8");

        static_assert(
            std::is_same_v<
                CutlassWeightType,
                int8_t> || std::is_same_v<CutlassWeightType, cutlass::int4b_t> || std::is_same_v<CutlassWeightType, cutlass::float_e4m3_t> || std::is_same_v<CutlassWeightType, cutlass::float_e5m2_t>,
        ...
        static_assert(std::is_same_v<EpilogueTag, lyradiff::cutlass_extensions::EpilogueOpBias>, "");
        using EVT_bias_addition = cutlass::epilogue::fusion::Sm90EVT<
            ...
            cutlass::epilogue::fusion::Sm90ColBroadcast<0,
                                                        TileShape,
                                                        CutlassBiasType,
                                                        Stride<_1, _0, _0>,
                                                        AlignmentBias>  // bias
        ...
        using CollectiveEpilogue = typename cutlass::epilogue::collective::CollectiveBuilder<
            ...
            void,
            typename cutlass::layout::LayoutTranspose<LayoutBias>::type,
            AlignmentBias,
            CutlassOutputType,
        ...
        if (occupancy != nullptr) {
            *occupancy = lyradiff::cutlass_extensions::compute_occupancy_for_kernel<GemmKernel, true>();
            return;
        }
        ...
        if constexpr (cutlass::isFinegrained(QuantOp)) {
            ...
            if (group_size % cta_shape_k != 0) {
                ...
                throw std::runtime_error("[LyraLLMs Error][fpA_intB Runner]" + err_msg);
            }
        ...
        typename Gemm::Arguments args{
            ...
            {n, m, k, 1},
            {reinterpret_cast<CutlassWeightType const*>(B),
             stride_B,
             reinterpret_cast<CutlassActivationType const*>(A),
             stride_A,
             reinterpret_cast<ElementScale const*>(weight_scales),
             stride_S,
             group_size,
        ...
        if (gemm.get_workspace_size(args) > workspace_bytes) {
            LYRA_LOG_ERROR("[LyraLLMs Error][fpA_intB Runner] given workspace size insufficient.");
        }
        ...
        throw std::runtime_error("[LyraLLMs Error][fpA_intB Runner] Config not compiled with FAST_BUILD.");
    }
#endif  // FAST_BUILD

#else   // COMPILE_HOPPER_MIXED_INPUT_GEMMS
    throw std::runtime_error(
        "[LyraLLMs Error][fpA_intB Runner] Please recompile with support for hopper by passing 90-real as an arch "
        "to build_wheel.py.");
