...
#include "src/lyradiff/kernels/cutlass_kernels/fpA_intB_gemm/launchers/fpA_intB_launcher_sm90.h"
...
void sm90_dispatch_epilogue_schedules(ActivationType const*  A,
                                      WeightType const*      B,
                                      ...
                                      int*                   occupancy = nullptr)
{
    ...
    switch (gemm_config.epilogue_schedule) {
        case tkc::EpilogueScheduleType::AUTO:
            ...
            sm90_generic_mixed_gemm_kernelLauncher<ActivationType,
                                                   WeightType,
                                                   ScaleZeroType,
                                                   BiasType,
                                                   OutputType,
        ...
        default:
            throw std::runtime_error(
                "[lyradiff Error][fpA_intB][sm90_dispatch_epilogue_schedules] epilogue schedule config is invalid for "
                "mixed "
                "type GEMM.");
            break;
    }
...
template<typename ActivationType,
         ...
         typename CTAShape,
         typename ClusterShape>
void sm90_dispatch_mainloop_schedules(ActivationType const*  A,
                                      WeightType const*      B,
                                      ScaleZeroType const*   weight_scales,
                                      ScaleZeroType const*   weight_zero_points,
                                      BiasType const*        biases,
    ...
    if constexpr (tile_shapes_supported) {
        switch (gemm_config.mainloop_schedule) {
            case tkc::MainloopScheduleType::AUTO:
                ...
                sm90_dispatch_epilogue_schedules<ActivationType,
                                                 WeightType,
                                                 ScaleZeroType,
                                                 BiasType,
                                                 OutputType,
            ...
            default:
                throw std::runtime_error(
                    "[lyradiff Error][fpA_intB][sm90_dispatch_mainloop_schedules] mainloop schedule config is invalid "
                    "for "
                    "mixed type GEMM.");
                break;
        }
        ...
        throw std::runtime_error(
            "[lyradiff Error][fpA_intB][sm90_dispatch_mainloop_schedules] Unsupported CTA and Cluster shapes for "
            "mixed type GEMM.");
    }
}
...
template<typename ActivationType,
         ...
         typename EpilogueTag,
         typename CTAShape>
void sm90_dispatch_gemm_config(ActivationType const*  A,
                               WeightType const*      B,
                               ScaleZeroType const*   weight_scales,
                               ScaleZeroType const*   weight_zero_points,
                               BiasType const*        biases,
                               ...
                               int*                   occupancy = nullptr)
{
    ...
    switch (gemm_config.cluster_shape) {
        case tkc::ClusterShape::ClusterShape_1x1x1:
            sm90_dispatch_mainloop_schedules<ActivationType,
                                             WeightType,
                                             ScaleZeroType,
                                             BiasType,
                                             OutputType,
        ...
        default:
            throw std::runtime_error(
                "[lyradiff Error][fpA_intB][dispatch_CGA_config] Config is invalid for mixed type GEMM.");
            break;
    }
}
...
template<typename ActivationType,
         ...
         cutlass::WeightOnlyQuantOp QuantOp,
         typename EpilogueTag>
void sm90_dispatch_gemm_to_cutlass(ActivationType const*  A,
                                   WeightType const*      B,
                                   ScaleZeroType const*   weight_scales,
                                   ScaleZeroType const*   weight_zero_points,
                                   BiasType const*        biases,
                                   ...
                                   int*                   occupancy = nullptr)
{
    ...
    switch (gemm_config.tile_config_sm90) {
        case tkc::CutlassTileConfigSM90::CtaShape64x16x128B:
            sm90_dispatch_gemm_config<ActivationType,
                                      WeightType,
                                      ScaleZeroType,
                                      BiasType,
                                      OutputType,
        ...
        case tkc::CutlassTileConfigSM90::Undefined:
            throw std::runtime_error(
                "[lyradiff Error][fpA_intB][sm90_dispatch_gemm_to_cutlass] gemm config undefined.");
            break;
        case tkc::CutlassTileConfigSM90::ChooseWithHeuristic:
            throw std::runtime_error(
                "[lyradiff Error][fpA_intB][sm90_dispatch_gemm_to_cutlass] gemm config should have already been set by "
                "heuristic.");
            break;
        default:
            throw std::runtime_error(
                "[lyradiff Error][fpA_intB][sm90_dispatch_gemm_to_cutlass] Config is invalid for mixed type GEMM.");
            break;
    }
}
